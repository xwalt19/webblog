...
const Camps: React.FC = () => {
  ...
  const { data: camps, isLoading, isError, error } = useQuery<SupabaseCamp[], Error>({
    queryKey: ['campsPublic'], // Unique key for public camps
    queryFn: async () => {
            ...
            <Card key={camp.id} className="flex flex-col shadow-lg hover:shadow-xl transition-shadow duration-300">
              <CardHeader className="pb-4">
                ...
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                  ...
                  <span>{formatDynamicScheduleOrDates(camp.dates)}</span>
                </div>
              </CardHeader>
