...
const UploadYouTubeVideo: React.FC = () => {
  ...
  const fetchVideoData = async (id: string) => {
    ...
    try {
      ...
      toast.error(t("upload youtube video.fetch error", { error: err.message }));
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!title || !videoUrl || !publishedAt) {
      toast.error(t("upload youtube video.required fields missing"));
      setUploading(false);
      return;
    }
    ...
    try {
      ...
      toast.success(videoId ? t("upload youtube video.video updated successfully") : t("upload youtube video.video added successfully"));
      ...
      toast.error(t("upload youtube video.save failed", { error: err.message }));
    } finally {
      setUploading(false);
    }
  };
    ...
    <div className="container mx-auto py-10 px-4">
      <section className="text-center mb-12">
        <h1 className="text-4xl md:text-5xl font-bold mb-4 text-primary">
          {videoId ? t('upload youtube video.edit video title') : t('upload youtube video.add video title')}
        </h1>
        <p className="text-lg text-muted-foreground max-w-3xl mx-auto">
          {videoId ? t('upload youtube video.edit video subtitle') : t('upload youtube video.add video subtitle')}
        </p>
      </section>
      ...
      <Card className="max-w-3xl mx-auto p-6 md:p-8 shadow-lg">
        <CardHeader className="text-center pb-6">
          <CardTitle className="text-2xl font-bold mb-2">
            {videoId ? t('upload youtube video.edit video') : t('add new youtube video')}
          </CardTitle>
          <CardDescription className="text-muted-foreground">
            {videoId ? t('upload youtube video.fill form to edit video') : t('fill form to add new video')}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <Label htmlFor="title">{t('upload youtube video.title label')}</Label>
              <Input
                id="title"
                type="text"
                placeholder={t('upload youtube video.title placeholder')}
                value={title}
            ...
            <div>
              <Label htmlFor="description">{t('upload youtube video.description label')}</Label>
              <Textarea
                id="description"
                placeholder={t('upload youtube video.description placeholder')}
                value={description}
            ...
            <div>
              <Label htmlFor="videoUrl">{t('upload youtube video.video url label')}</Label>
              <Input
                id="videoUrl"
                type="url"
                placeholder={t('upload youtube video.video url placeholder')}
                value={videoUrl}
            ...
            <div>
              <Label htmlFor="publishedAt">{t('upload youtube video.published at label')}</Label>
              <Popover>
                <PopoverTrigger asChild>
                  ...
                  >
                    ...
                    {publishedAt ? format(publishedAt, "PPP") : <span>{t('upload youtube video.pick a date')}</span>}
                  </Button>
                </PopoverTrigger>
            ...
            <div>
              <Label htmlFor="thumbnail-upload">{t('upload youtube video.thumbnail label')}</Label>
              <Input
                id="thumbnail-upload"
                type="file"
                accept="image/*"
              ...
              {thumbnailFile ? (
                <p className="text-sm text-muted-foreground mt-2">
                  {t('upload youtube video.selected thumbnail')}: {thumbnailFile.name}
                </p>
              ) : initialThumbnailUrl && (
                <p className="text-sm text-muted-foreground mt-2">
                  {t('upload youtube video.current thumbnail')}: <a href={initialThumbnailUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">{initialThumbnailUrl.split('/').pop()}</a>
                </p>
              )}
            </div>
            <Button type="submit" className="w-full" disabled={uploading}>
              {uploading ? t('uploading') : (videoId ? t('upload youtube video.save changes') : t('upload youtube video.submit button'))}
            </Button>
          </form>
        </CardContent>
      </Card>
      ...
      <div className="text-center mt-12">
        <Link to="/admin/manage-youtube-videos">
          <Button>{t('back to youtube videos list')}</Button>
        </Link>
      </div>
    </div>
  );
